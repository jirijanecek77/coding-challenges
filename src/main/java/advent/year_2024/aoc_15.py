...
from utils import line_generator, Position, move_up, move_down, move_left, move_right, Direction
...
def push_up_down(pos: Position, move_fn, playground: list[list[str]]) -> Position:
    ...
    next_new_pos = move_left(new_pos) if playground[new_pos.row][new_pos.col] == "]" else move_right(new_pos)
...
def perform_move(move: Direction, playground: list[list[str]], pos: Position) -> Position:
    match move:
        case Direction.UP:
            new_pos = push_up_down(pos, move_up, playground)
        case Direction.DOWN:
            new_pos = push_up_down(pos, move_down, playground)
        case Direction.LEFT:
            new_pos = push_left_right(pos, move_left, playground)
        case Direction.RIGHT:
            new_pos = push_left_right(pos, move_right, playground)
        case _:
            raise ValueError
...
def solve_01():
    with open(filename) as file:
        ...
        result = sum(sum(col_index + 100 * row_index for col_index, val in enumerate(row) if val == "[") for row_index, row in enumerate(playground))
